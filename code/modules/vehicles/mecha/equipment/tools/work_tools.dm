...
/obj/item/mecha_parts/mecha_equipment/hydraulic_clamp/action(mob/living/source, atom/target, list/modifiers)
	...
	if(ismecha(target))
		...
		if(!source.a_intent == INTENT_HARM)
			...
			log_combat(source, M, "removed both arms with a real clamp,", "[name]", "(COMBAT MODE: [uppertext(source.a_intent == INTENT_HARM)] (DAMTYPE: [uppertext(damtype)])")
			return ..()
		...
		log_combat(source, M, "attacked", "[name]", "(Combat mode: [source.a_intent == INTENT_HARM ? "On" : "Off"]) (DAMTYPE: [uppertext(damtype)])")
	return ..()
